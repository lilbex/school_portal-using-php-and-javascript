...
class users
{
	...
	{
		$query=$this->conn->query("select * from question where cat_id='$ques' ORDER BY rand() ");
		while($row=$query->fetch_array(MYSQLI_ASSOC))
		{
			$this->ques[]=$row;
		}
	...
	public function post_answer($st_id,$st_class,$st_score,$st_attempt)
	{
		$post_answer = "insert into score($st_id,$st_class,$st_score,$st_attempt) value('st_id,st_class,st_score, no_attempt)";
		$post_answer_run = $this->connectdb->query($post_answer);
		return $post_answer_run;
	}	
	public function update_answer($st_id,$st_class,$st_score,$st_attempt)
	{
		$post_answer = "update score set st_class='st_class', st_score='st_score', st_attempt='no_attempt' where st_id='st_id'";
		$post_answer_run = $this->connectdb->query($post_answer);
		return $post_answer_run;
